...
async function fetchData(params: { categories_tags: string; page_size: number }) {
  const url = `${apiUrl}?categories_tags=${params.categories_tags}&page_size=${params.page_size}`;
...
export const useBeers = (params?: { categories_tags: string; page_size: number }) => {
